 ...
 *
 * Both a light ("hell") and a dark ("dunkel") variant are provided and both
 * use the same Wien brand palette, Wien Rot as the primary colour and the
 * Wiener Melange font stack.
 */
import {
  UnifiedTheme,
  createUnifiedTheme,
...
const pageThemes = {
  home: genPageTheme({ colors: [wienColors.wienRot, wienColors.morgenrot], shape: shapes.wave }),
...
/** Stadt Wien light ("hell") theme. */
export const wienLightTheme: UnifiedTheme = createUnifiedTheme({
  fontFamily: wienFontStack,
  ...
  components: {
    ...
    MuiAppBar: {
      styleOverrides: {
        root: { backgroundColor: wienColors.wienRot, color: wienColors.weiss },
      },
    },
...
/** Stadt Wien dark ("dunkel") theme. */
export const wienDarkTheme: UnifiedTheme = createUnifiedTheme({
  fontFamily: wienFontStack,
  defaultPageTheme: 'home',
  pageTheme: pageThemes,
  typography,
