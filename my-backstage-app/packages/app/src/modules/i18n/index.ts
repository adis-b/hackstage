/**
 * Stadt Wien i18n module — registers German (de) translations for the most
 * visible core Backstage plugins. Provides a working Settings → Appearance
 * language toggle between Deutsch (de) and English (en).
 *
 * The available/default languages themselves are configured in
 * app-config.yaml under `app.extensions.api:app/app-language.config`.
 *
 * NOTE: The TranslationsApi input on the core app plugin is marked
 * `internal: true`, which means only extensions belonging to the same plugin
 * (`app`) are accepted. We therefore register the TranslationBlueprint
 * extensions via `createFrontendModule({ pluginId: 'app' })`.
 */
import {
...
const userSettingsDe = createTranslationMessages({
  ...
  messages: {
    ...
    'defaultSettingsPage.tabsTitle.authProviders': 'Authentifizierungs-Anbieter',
